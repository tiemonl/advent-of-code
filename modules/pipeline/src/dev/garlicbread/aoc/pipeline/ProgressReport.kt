...
fun createProgressTable(solvedMap: Map<Int, Int>): String = buildString {
    ...
    val totalPossible = solvedMap.keys.sumOf { maxPartsForYear(it) }
    val totalPct = if (totalPossible == 0) 0.0 else totalSolved.toDouble() / totalPossible * 100.0

    appendLine("- Total stars: **$totalSolved / $totalPossible** (${formatPercent(totalPct)} of attempted years)")
    appendLine()
    appendLine("### By Year")
    appendLine()
